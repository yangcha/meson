...
class TeeThread(threading.Thread):
    ...
    def run(self):
        ...
        for line in self.fd:
            chunks.append(line)
            self.out_fd.write(line.decode(sys.stdout.encoding))
...
class GitRetry:
    ...
    def __init__(self, retry_count=None, delay=None, delay_factor=None):
        ...
        self.delay = max(delay, 0) if delay else self.DEFAULT_DELAY_SECS
        self.delay_factor = max(delay_factor, 0) if delay_factor else self.DEFAULT_DELAY_FACTOR

    def shouldRetry(self, stderr):
        m = GIT_TRANSIENT_ERRORS_RE.search(stderr.decode(sys.stderr.encoding))
        if not m:
            return False
...
def main(args):
    ...
    parser.add_option('-D', '--delay-factor', metavar='FACTOR',
                      type=int, default=GitRetry.DEFAULT_DELAY_FACTOR,
